...
fname2 = 'BackS002581.csv'
data2 = pd.read_csv(ddir + fname2)
time2, flux2 = data2.Time, data2.NormalisedFlux # just extract the columns as variables

plt.title('Time-dependent flux of example stars')
plt.plot(time,flux,'.',markersize=16, label = 'Star 1')
plt.plot(time,[1 for i in time],'--',color = 'C0', label = 'Star 1 baseline')
plt.plot(time2,flux2-0.5,'.',markersize=16, label = 'Star 2', color = 'C1')
plt.plot(time2,[0.5 for i in time],'--',color = 'C1', label = 'Star 2 baseline')
plt.xlabel('Time (h)')
...
LS2 = LombScargle(time2,flux2)
power2 = LS2.power(freqs)

print('Best period: %.2f h' % (1/freqs[np.argmax(power)]))

plt.plot(freqs,power, label = 'Star 1')
plt.plot(freqs,power2-1,label = 'Star 2')
plt.xlabel('Frequency (cycles/h)')
plt.ylabel('Lomb-Scargle Power')
plt.title('Lomb-Scargle Periodogram of example stars')
plt.legend()
plt.show()
...
print(len(all_stars),'stars above 10 mas parallax') # check how many stars there are total with go

#ddir_stars = 'C:/Users/dylan/Documents/GitHub/DL-Proj/DATA' # point this to where you unzip your data!
ddir_stars = 'C:\\Users\\rgray\\OneDrive\\ryan\\Uni\\2023 sem 1\\PHYS3080\\Assignments\\DL-Proj\\DATA\\'
ddir = ddir_stars + '/Variable_Star_Data/'

variables = pd.read_csv(r'C:\Users\rgray\OneDrive\ryan\Uni\2023 sem 1\PHYS3080\Assignments\DL-Proj\Dylan\Main Files\CutoffVariables.csv')
...
plt.title('Period-Luminosity diagram of benchmark stars, with linear fits')
plt.show()
